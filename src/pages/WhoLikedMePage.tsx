...
import { Heart, Eye, Coins, Lock, ArrowLeft, CheckCircle } from "lucide-react";
...
export default function WhoLikedMePage() {
  ...
  const { data: likers = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Exclude people I already liked back (mutual = matched)
  ...
  const revealMutation = useMutation({
    mutationFn: async (profileUserId: string) => {
      ...
      const { error } = await supabase
        .from("wallets")
        .update({ balance: wallet.balance - REVEAL_COST })
        .eq("user_id", user.id);
      ...
      await supabase.from("transactions").insert({
        ...
        amount: -REVEAL_COST,
        type: "reveal",
        description: "Revealed who liked you",
      });
      ...
      <div className="sticky top-0 z-40 bg-background/80 backdrop-blur-xl border-b border-border px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-xl font-heading font-bold text-gradient">Who Liked You</h1>
            <p className="text-xs text-muted-foreground">{likers.length} people liked your profile</p>
          </div>
      ...
      <div className="p-4">
        {isLoading ? (
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center pt-20">
            ...
            <h3 className="text-lg font-heading font-semibold">No likes yet</h3>
              ...
              {likers.map((liker, index) => {
                ...
                return (
                  <motion.div
                    ...
                    className="relative aspect-[3/4] rounded-2xl overflow-hidden bg-secondary group"
                  >
                    {/* Photo */}
                    {liker.avatar_url && (
                      <img
                    ...
                    {isRevealed && (
                      <div className="absolute bottom-0 left-0 right-0 p-3">
                        ...
                        <div className="flex gap-2 mt-2">
                          ...
                          <button
                            onClick={() => navigate(`/user/${liker.user_id}`)}
